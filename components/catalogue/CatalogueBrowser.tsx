...
function CatalogueBrowserComponent({ products, categoryName }: Props) {
      ...
      <div className="sticky top-3 z-30 mx-auto mb-8 max-w-[1500px] px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-3 rounded-2xl border border-[#E8DCC8] bg-[#FAF8F5]/95 p-3 shadow-[0_12px_35px_rgba(27,27,27,0.08)] backdrop-blur md:flex-row md:items-center md:p-4">
          <div className="relative flex-1">
            ...
            <input id="catalogue-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search model number, material or design" className="h-12 w-full rounded-xl border border-[#E8DCC8] bg-white pl-11 pr-4 font-sans text-sm text-[#1B1B1B] placeholder:text-[#6B6258] focus:border-[#C89B3C] focus:outline-none focus-visible:ring-4 focus-visible:ring-[#C89B3C]/25" />
          </div>
          <div className="flex items-center gap-2">
            ...
            <select id="catalogue-sort" value={sort} onChange={(event) => setSort(event.target.value as SortMode)} className="h-12 min-w-0 flex-1 rounded-xl border border-[#E8DCC8] bg-white px-3 font-sans text-sm text-[#1B1B1B] focus:border-[#C89B3C] focus:outline-none focus-visible:ring-4 focus-visible:ring-[#C89B3C]/25 md:w-36 md:flex-none">
            ...
            <div className="flex h-12 rounded-xl border border-[#E8DCC8] bg-white p-1">
              <button type="button" onClick={() => setListView(false)} aria-label="Grid view" aria-pressed={!listView} className={`grid h-full w-10 place-items-center rounded-lg focus-visible:ring-4 focus-visible:ring-[#C89B3C]/25 ${!listView ? 'bg-[#1B1B1B] text-white' : 'text-[#4F463D]'}`}><Grid2X2 size={17} aria-hidden="true" /></button>
              <button type="button" onClick={() => setListView(true)} aria-label="List view" aria-pressed={listView} className={`grid h-full w-10 place-items-center rounded-lg focus-visible:ring-4 focus-visible:ring-[#C89B3C]/25 ${listView ? 'bg-[#1B1B1B] text-white' : 'text-[#4F463D]'}`}><List size={18} aria-hidden="true" /></button>
            </div>
          </div>
        </div>
      </div>
      <div className="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-center justify-between"><p className="font-sans text-sm text-[#4F463D]"><span className="font-semibold text-[#1B1B1B]">{visibleProducts.length}</span> designs available</p></div>
        {visibleProducts.length ? <div className={listView ? 'space-y-5' : 'grid gap-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4'}>{visibleProducts.map((product) => <ProductCard key={product.id} product={product} listView={listView} />)}</div> : <div className="rounded-3xl border border-dashed border-[#D7BE8E] px-6 py-20 text-center"><p className="font-serif text-2xl">No designs found</p><p className="mt-2 font-sans text-sm text-[#4F463D]">Try a model number, material, or another search term.</p></div>}
      </div>
    </section>
  );
}

const ProductCard = memo(function ProductCard({ product, listView }: { product: DoorProduct; listView: boolean }) {
...
export const CatalogueBrowser = memo(CatalogueBrowserComponent);
